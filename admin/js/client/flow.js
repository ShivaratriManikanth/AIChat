...
function addNodeOption(nodeIndex) {
  if (!flowNodes[nodeIndex].config) flowNodes[nodeIndex].config = {};
  if (!flowNodes[nodeIndex].config.options) flowNodes[nodeIndex].config.options = [];
  flowNodes[nodeIndex].config.options.push('New Option');
  renderFlowNodes();
  saveFlow(false).then(() => { openFlowPreview(); });
}

function deleteNodeOption(nodeIndex, optIndex) {
  if (!flowNodes[nodeIndex].config || !flowNodes[nodeIndex].config.options) return;
  flowNodes[nodeIndex].config.options.splice(optIndex, 1);
  renderFlowNodes();
  saveFlow(false).then(() => { openFlowPreview(); });
}

function updateNodeOption(nodeIndex, optIndex, value) {
  if (!flowNodes[nodeIndex].config || !flowNodes[nodeIndex].config.options) return;
  flowNodes[nodeIndex].config.options[optIndex] = value.trim();
  saveFlow(false).then(() => { openFlowPreview(); });
}
          ...
          <div style="display:flex; flex-direction:column; gap:6px;">
            ${opts.map((opt, optIndex) => `
              <div style="display:flex; gap:6px; align-items:center;">
                <input type="text" class="flow-node-input" style="margin-top:0;" value="${opt}" onchange="updateNodeOption(${i}, ${optIndex}, this.value)" placeholder="Option #${optIndex + 1}">
                <button class="flow-node-btn delete" onclick="deleteNodeOption(${i}, ${optIndex})" style="padding: 7px 10px; flex-shrink: 0; background:rgba(239, 68, 68, 0.1); color:#f87171; border:none; border-radius:6px; cursor:pointer;" title="Delete Option" type="button">🗑️</button>
              </div>
            `).join('')}
            <button class="flow-node-btn" onclick="addNodeOption(${i})" style="padding:6px 12px; font-size:11px; align-self:flex-start; margin-top:2px; border:1px solid #6366f1; color:#a5b4fc; background:transparent; font-weight:600;" type="button">➕ Add Option</button>
          </div>
        </div>
      `;
